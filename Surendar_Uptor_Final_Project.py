...
plt.figure(figsize=(10,6))
sns.scatterplot(x=df['imdb_votes'], y=df['imdb_score'], alpha=0.6, color='blue')
plt.plot(df['imdb_votes'], intercept + slope * df['imdb_votes'], color='red', label="Regression Line")
plt.xlabel("IMDb Votes")
plt.ylabel("IMDb Score")
plt.title(f"IMDb Votes vs IMDb Score (r={r_value:.2f})")
plt.legend()
plt.show()
...
if 'genres' not in df.columns:
    raise ValueError("Column 'genres' not found in dataset!")

"""Clean and explode genre data (handling multiple genres per movie)"""
df['genres'] = df['genres'].astype(str).str.strip("[]").str.replace("'", "").str.split(", ")
df = df.explode('genres')
...
"""Print the top 10 genres to verify correct data"""
print("Top 10  Common Movie Genres:")
print(genre_counts)
